...
\label{sec:pairs}
Table~\ref{tab:baselines} puts the two standard answers to label noise, and our earlier tracklet vote, beside ours. Label smoothing leaves the detector where the released labels leave it out of distribution, \BaseLsOODGain{} points and positive in \BaseLsOODPos{} of \SvCells{} cells: absorbing the noise does not recover a wrong type id. Confidence relabelling moves the labels the most and the scores the furthest in the wrong direction, \BaseCfOODGain{} points out of distribution. It rewrites \ConfidChangedPct\% of the ids and \ConfidVsSceneDisagreePct\% of those disagree with the type the geometry votes for: a detector trained on the released labels is not an independent witness to them. Under the preregistered rule the scene vote is the only arm that gains out of distribution in every cell, \BaseSvOODGain{} points, against \BaseTrkOODGain{} in \BaseTrkOODPos{} of \SvCells{} for the adjacent-frame vote. At threshold \ConfidTauHigh{} (post hoc, accelerator A only) it changes \ConfidTauHighChangedPct\% of the ids, about as many as our vote, and still falls \ConfidTauHighOODDrop{} points out of distribution in all \ConfidTauHighCells{} cells while beating us on the standard split: fewer edits soften the loss without turning it into a gain. Smoothing is the better choice on the standard split, ahead of us in every cell, so the case for relabelling is the out-of-distribution one. The last epoch keeps the order: \LastSvOODGain{} for the scene vote against \LastLsOODGain, \LastCfOODGain{} and \TrkLSixOODVsRawGainL{} for the other three.
...
\section{Conclusion}
GlassNICOL's automatic labels disagree on the same glass in \FlipRateReg\% of adjacent-view pairs; its hand labels agree. Voting over a scene's views changes \SceneChangedPct\% of type ids without moving a box, and under the preregistered rule ours alone gains in every cell out of distribution, \SvSixOODVsRawGain{} six-class AP.
